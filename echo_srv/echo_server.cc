...
string Echo_server::create_serv_handshake(){
	...
	response+="Sec-WebSocket-Accept: "+gen_accept_key(cli_info[i].websock_key) + "\r\n";
...
{	
	open_max_ =mx_open; 
	cli_info = new cli_request[mx_open+1]; 
	client = new pollfd[mx_open+2];
	maxi_ = 0;
	clicount = -1;
	...
	for(int i=0;i<open_max_+2;++i)
		cli_info[i].checked = false;
}
		...
		for(i=1; i<=maxi_; i++){
			...
			if (client[i].revents & (POLLRDNORM | POLLERR)){
				...
				if(ioctl(client[i].fd, FIONREAD, &size)>=0){
					...
					if(cli_info[i].checked){
						cout << "read" <<endl;
						n = read(client[i].fd, &buf[0], 2014);
						cout << n << endl;
					}else n = recv(client[i].fd, &buf[0], size, 0);

					if (n<0){

						if (errno == ECONNRESET){
							close_cli_conn(i);
						} else if(cli_info[i].checked){
							perror("read error");
						...
						if(cli_info[i].checked){
							...
							if(write(client[i].fd, &buf[0], size)<0)
								perror("write");

						}else if(parse_request(buf)){
								
							string response = create_serv_handshake();
							if ((send(client[i].fd, &response[0], response.length(), 0)) < 0){
								perror("send() error");
							}
